...
import { BackupJob, BackupFormData } from './types';
...
interface ScheduledBackupsTabProps {
  instances: any;
  postgresInstances: any;
  vpsInstances: any;
}

export default function ScheduledBackupsTab({ 
  instances, 
  postgresInstances, 
  vpsInstances 
}: ScheduledBackupsTabProps) {
  const { user } = useAuth();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <div>
          <h2 className="text-xl font-semibold text-gray-900">Automated Backup Jobs</h2>
          <p className="mt-1 text-sm text-gray-600">
            Schedule and manage automated backups for object storage
          </p>
        </div>
        <button
          onClick={() => {
